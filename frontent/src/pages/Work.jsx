  ...
  return (
    ...
    <div className='flex items-center justify-between '>
        ...
        <div className='flex  items-center gap-6 '>
            <Link to={"/work"}>
            <img src="/public/svgs/logo.svg" className="sm:mx-auto md:mx-start block" width={100} height={100} alt="" />
            </Link>
...
{WorkData?.map((item,index)=>{
    return <li  className={` flex items-center whitespace-nowrap gap-2 relative  text-sm  group   font-semibold `} key={index}>
        {item?.title && <>
...
{item?.list && <IoIosArrowDown className="text-gray-600  group-hover:rotate-180 transition-all duration-300"/> }
...
<ul className="absolute  list_style my-3   left-0 bg-white shadow-xl p-2  rounded-md  gap-4  hidden group-hover:block hover:text-black  top-full w-58">
    ...
    {item?.list?.map((item2,index2)=>{
        return <li key={index2} className="mb-3">{item2?.title}</li>
    })}
</ul>
